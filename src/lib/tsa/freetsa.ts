...
export interface TsaRequestOptions {
  ...
  maxRetries?: number
  baseDelayMs?: number
  /** Per-attempt fetch timeout in ms. Default 10s. */
  timeoutMs?: number
}
 ...
 *
 * Each fetch attempt has its own AbortController-driven timeout (default
 * 10s, configurable via opts.timeoutMs). Without this, a hung TCP
 * connection to the TSA could exhaust the 300s Vercel function budget
