...
- D.3 perf: `verifyCertificate` now does an O(1) `Set.has()` membership check against a stored leaf set instead of recomputing the day's Merkle root on every verify. Migration `00009_daily_log_leaf_set.sql` adds a `leaf_hashes JSONB` column to `evidence_log_entries` with a backfill UPDATE for any pre-existing log entries.
- D.3 cron: new `/api/cron/retry-phase-a` route + `EvidenceService.retryPhaseA` method recovers certs whose Phase A blob upload failed (previously surfaced as `pdf_storage_path IS NULL` with no recovery path). Idempotent: skips certs that already have a PDF.
- D.3 cron: per-attempt fetch timeout (default 10s) on the FreeTSA client so a hung TCP connection cannot exhaust the backfill cron's 300s function budget.
- D.3 internal: extracted `isAuthorized` to `src/lib/cron.ts`; extracted nullable `toBuffer` helper to `src/lib/buffers.ts` (corrects a latent bug where the inline pattern returned `Buffer.alloc(0)` for null inputs); folded `content_hash_at_send` write into the existing ownership-check SELECT in `DocumentService.sendForSigning`; removed dead pass-through wrappers in `EvidenceService`; replaced hand-rolled `stableStringify` in `src/lib/canonicalize.ts` with `JSON.stringify` + sorted keys. Renamed `VerificationResult.tsaPending` to `tsaTokenMissing` to match its actual semantic.
- D.3 docs: `SECRETS.md` documents `CRON_SECRET` and `TSA_URL`. `scripts/smoke-cron.sh` verifies the three cron endpoints.
